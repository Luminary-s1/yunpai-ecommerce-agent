...
初版规则先按 `_RULE_PRIORITY` 取候选；只有命中词被局部否定，或 `曝光` / `推荐` /
`物流` 出现在列举出的多义上下文时才交给模型复核。普通规则命中仍直接返回，风险
提示与换措辞 few-shot 也只附加在复核路径。
...
## cross-domain 正向业务证据门

初版的非业务上下文表只会记住见过的反例。现行实现改为相反的有限判据：对已经证实
多义的 `曝光` / `推荐` / `物流` / `退款`，有实义上下文时必须同时出现该业务动作
所需的商品目录、订单履约、退款处理或商家服务证据，规则层才可直接返回；证据不足
时交给模型。证据只在命中词所在分句内生效；`曝光` 还必须同时具备投诉动作与电商
对象两组证据。

纯业务短句没有上下文证据，因此另走正向的请求包装语法：去掉命中词后，所在分句只
能剩明确动作前缀或语气后缀（如“我要…”、“…一下”、“…呢”）才保留规则快路径；
剩下任意实义内容仍回到证据门。否定检测最先执行，疑问式“可以 / 请问 / 吗”不算
明确动作，避免把售前退款政策重新短路成售后。

独立路由留出集在 `cross_domain_holdout.jsonl`。运行器只用确定性 probe model
观察 `method`，不评价模型最终语义，所以不会受 provider 可用性影响，也不会把
`default → chitchat` 当成答对：

```bash
.venv/bin/python evals/intent/run_cross_domain.py \
  --out evals/intent/runs/20260804-cross-domain.json
```

| 路由指标 | 证据门前 | 仅证据门 | 短句修复后 |
|---|---:|---:|---:|
| 跨域表达进入模型（首批，已泄漏） | 33.3%（2/6） | 100.0%（6/6） | 100.0%（6/6） |
| 跨域表达进入模型（第二批，已泄漏） | — | 100.0%（6/6） | 100.0%（6/6） |
| 明确业务表达保留规则快路径 | 100.0%（8/8） | 100.0%（8/8） | 100.0%（8/8） |
| **高频短句保留规则快路径** | **100.0%（5/5）** | **0.0%（0/5）** | **100.0%（5/5）** |

短句修复前，`我要退款` / `退款` / `物流呢` / `推荐一下` / `曝光你们` 会在模型
不可用时降级为 `chitchat`。修复后五条恢复 `rule / 0.95 / error=None`；四条短跨域
设计守卫与三条售前政策问句仍交模型。短跨域和政策问句是实现守卫，不冒充新的留出
泛化证据。

证据文件为 `runs/20260804-cross-domain-before.json` 与
`runs/20260804-cross-domain-after.json`。生产证据表不包含医院、景点、照片、摄影、
行业或公司等留出集反例域词；这次提升来自正向业务契约，不是扩充反例黑名单。

短句修复后的 25 条路由结果为
`runs/20260805-cross-domain-terse-fix.json`；原 52 条 rule 结果为
`runs/20260805-rule-after-terse-fix.json`，判定准确率仍为 15/15、`plain` 仍为
22/22。

原 52 条 rule 基准仍为规则判定 15/15、`plain` 22/22；与旧结果共同的 51 个 ID
路由零变化，另一个 ID 是此前已经人工裁定的 `as-013 → pi-014`，与本次实现无关。
当前进程健康检查为 `disabled`，且本地没有模型配置，因此未生成新的 live 成绩；
路由已验证，六条留出表达的模型语义准确率仍需在有效配置下补测。
...
- **警惕单指标归因**：同次运行 `after_sales` 召回 61.5% → 83.3%，若只看这一个
  数会得出「已修复」的结论；实际涨幅主要来自 1305 缺测由 16 条降至 9 条。凡是
  跨运行比较，一律走「平台不稳时怎么比较两次运行」一节。
...
`cross_domain_holdout.jsonl` 里带 `leaked: true` 的条目**不再是泛化证据**：它们
一旦写进工作记录或复核意见，答案就先于考试公开了，后续实现可以直接照着它们编码。
保留是为了回归，不是为了评估。

因此每次验收泛化能力，都必须现造一批双方都没用过的探针，并在验收后同样标记为
已泄漏。这是留出集的固有成本，不是流程缺陷。

同理，`business-guard` / `terse-business-guard` 两组守卫的作用是**约束修复的代价**：
把跨域误判交给模型是有代价的，守卫负责保证这个代价没有转嫁到高频业务路径上。
只看跨域召回率而不看业务快路径保留率，会把「全部交给模型」当成满分方案。
