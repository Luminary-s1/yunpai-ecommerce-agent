...
多义的 `曝光` / `推荐` / `物流` / `退款`，命中词必须同时出现该业务动作所需的
商品目录、订单履约、退款处理或商家服务证据，规则层才可直接返回；证据不足时交给
模型。证据只在命中词所在分句内生效；`曝光` 还必须同时具备投诉动作与电商对象两组
证据。否定检测、`_RULE_PRIORITY` 与其他无歧义关键词均未改变。
...
| 路由指标 | 修改前 | 修改后 |
|---|---:|---:|
| 跨域表达进入模型（首批，已泄漏） | 33.3%（2/6） | 100.0%（6/6） |
| 跨域表达进入模型（第二批留出） | — | 100.0%（6/6） |
| 明确业务表达保留规则快路径 | 100.0%（8/8） | 100.0%（8/8） |
| **高频短句保留规则快路径** | **100.0%（5/5）** | **0.0%（0/5）** |

最后一行是本次改动的代价，也是不予收口的原因：`我要退款` / `退款` / `物流呢` /
`推荐一下` / `曝光你们` 由 `rule / 0.95` 转为需模型调用，模型不可用时降级为
`chitchat`。判据「无业务证据 → 交模型」对短句不成立——短句本就无证据可言。
应改为「存在跨域信号时才要求业务证据」。
...
- `after_sales` 召回偏低，模型倾向把带情绪的售后判成 `complaint`。口径已写进
  ...
  原句）。
- **`cross_domain` 泛化已达成但不收口**：高频短句业务消息 5/5 退化为需模型调用，
  provider 抖动时 P0 售后消息会变成闲聊，代价高于原误判。见「cross-domain 正向
  业务证据门」表末行。
