...
- 结论：2026-08-08 的 D24 修复候选仍不签署 M4 本机独立验收，证据 ID 为 E-20260808-003。FIX-11 已恢复非复核消息的规则零模型短路（`rule / 0.95`）；只有规则未命中或流程追责/业务证据冲突才进入模型仲裁，`退货/保修` 的责任追问有窄口径证据组，普通退货请求仍走规则。FIX-12 为唯一目录候选的已装配知识增加有界一步规划：模型仍生成并保 grounding，禁止工具循环，K3 不再 `react_step_limit_reached`。冻结 50 例与门禁阈值未改；FIX-12 后 mock WP4 `0.940 / hallucination_rate=0.020 / severe_failures=3 / passed`，同口径 live `deepseek-v4-flash` `0.820 / 0.060 / severe_failures=3 / passed`（complaint `7/8`、product `14/15`、after-sales `6/12`），两份新报告均保留。live 与旧报告差异如实记录，不宣称分类或回答能力提升。泄漏回归仅作回归：40 条 `31/40=77.5%`、覆盖率 `80%`，投诉平衡集 precision `100%`、recall `75%`、负例误报 `0/20`，不能替代密封新留出集。四场景延迟分解报告为 `p50=16297.7ms / p95=33594.4ms`（K3 `20390.2ms`，无工具调用），只代表泄漏场景且 provider 尾延迟仍是 P1；不得宣称全链路已变快。FIX-14 的分类层 gate 位置尚待负责人裁定，FIX-15 密封留出集尚未由验收人提供；浏览器 PNG 仍未更新，生产放行继续阻塞。
