...
| F-101 渠道适配器 SDK | P0 | 进行中 | F-001、F-002 | 定义能力版本、验签、标准消息、发送、上下文、限流、错误、去重、回放契约并通过模拟适配器测试 | 0.16.0 淘宝实现增加入站事件/Agent 任务事务、租约恢复、精确事件下游和异步回执观测；通用 SDK 抽取仍待第二渠道验证，见 E-20260722-002 |
| F-102 首个合法客服渠道连接器 | P0 | 已阻塞 | F-101、真实权限、测试账号 | 真实渠道可稳定收发、验签、去重、顺序控制、回执和审计；连续影子期无跨会话和静默丢消息 | 阻塞于渠道/服务商能力清单和授权 |
| F-103 统一渠道会话与可信上下文扩展 | P0 | 进行中 | F-002、F-101 | 消息信封覆盖租户、店铺、渠道、会话和多消息类型；context 在 checkpoint 前白名单化，跨租户/店铺不可合并 | 0.16.0 将脱敏入站事件、稳定会话、Agent invocation 和 context snapshot 按租户/店铺/事件关联；真实渠道多消息类型仍待联调，见 E-20260722-002 |
| F-104 平台/行业/店铺/商品/进化分层知识 | P2 | 进行中 | F-004、F-103 | 五层知识支持租户、店铺、SKU、版本、来源、审批、灰度和回滚；检索返回稳定证据 ID | 0.9.0 已完成分层、版本、评测、审批、激活、退役、回滚、租户隔离和检索过滤；灰度发布待 F-113，见 E-20260721-011 |
| F-105 SOP DSL 与版本化执行引擎 | P2 | 进行中 | F-003、F-104 | SOP 支持触发、上下文、步骤、守卫、转人工、后置条件、评测、发布和回滚；运行会话固定版本 | 0.13.0 已完成 schema v13 多步账本、审批、读取重试、写入未知态、补偿、恢复和管理处置；真实业务写工具/读回补偿与渠道灰度待验收，见 E-20260721-015 |
| F-106 ContextBuilder 与商品/SKU 顾问 | P2 | 进行中 | F-103、F-104 | 可信会话、商品/SKU、SOP、知识和工具目录按固定顺序组装；支持商品识别、推荐、对比和证据引用 | 0.15.0 已完成固定装配、版本证据、父子快照、冲突降级和完整性校验；商品实体识别/对比顾问仍待真实商品数据，见 E-20260722-001 |
| F-107 四种人机协同接待模式 | P2 | 进行中 | F-005、F-105 | 仅提示、人机协同、智能转接、夜间值守可按店铺/SOP 灰度；人工暂停后 AI 不再发送 | 0.16.0 将 shadow/assist/collaborative/automatic 接入真实入站任务；shadow 在 Graph 和落地层均零副作用，草稿/转人工/发送二次检查 owner 与策略；夜间值守和 SOP 级策略仍待完成，见 E-20260722-002 |
| F-108 多模态客服证据 | P2 | 候选 | F-103、商品数据、OCR/多模态服务 | 支持商品图、商品卡、尺码/参数表和售后图片结构化识别；高风险动作不以图片单独放行 | `EPIC-MULTIMODAL` |
| F-109 人工客服工作台 | P0 | 进行中 | F-005、F-107、前端 | 可查看建议/证据/SOP/风险，暂停、接管、修改发送、恢复、转队列并保留 diff 和反馈 | 0.22.2 增加运营/模拟/评测范围切换、来源标签、决策详情和上下文证据；真实渠道回执和坐席业务验收待联调，见 E-20260722-011 |
...
| F-117 版本化客户 Agent 评测 | P0 | 进行中 | F-001、F-103、F-113、客户脱敏标注 | 脱敏多轮标注集具备不可变版本、完整性哈希、实际 Agent 隔离运行、意图/转人工/证据/严重错误/回归指标、并发幂等、恢复和发布关联；客户代表性样本与真实模型基线通过业务签收 | 0.18.0 本地结构、API、后台和运行验收通过；真实客户数据、真实模型基线和业务阈值待验收，见 E-20260722-004 |
...
| F-308 营销与内容模块 | P1 | 已规划 | F-305、F-307、审批 | 提供广告盈利诊断和内容草稿/事实检查；不自建实时竞价，预算与发布必须审批 | `EPIC-CONTENT-AGENT` |
| F-309 利润与对账模块 | P1 | 已规划 | F-306、F-307、成本口径 | 提供经营利润、费用归集草案和差异任务，不替代财务总账或报税 | `EPIC-METRICS` |
