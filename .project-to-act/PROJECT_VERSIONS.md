...
- 占号状态：PR #10 已合入 main `1906365`，schema **v33** 在 `main`（knowledge_key 唯一索引 + retrieval_logs）。F-322 **v32** 已在 main。PR #11 的 `_apply_v31`（workspace 会话表）仍占用中、未合入。M7-R WP1 的功能提交 `0b54a24` 已把 **v34** 合入 main；本地 main `d35bfe1` 已预留 **v35** 给 M7-R WP3，运行迁移候选在分支提交 `6f0b116`，尚未合入 main。合 #11 时必须 31+32+33+34+35 五块并存，扫描 `MERGE-GATE PR-11`。下一空闲号 **36**。
- 最后更新：2026-08-18

## M7-R WP3 商品身份与对账（未单独升应用版）

- 状态：开发自测候选为 `6f0b116`，schema v35 预留在本地 main `d35bfe1`；运行代码尚未合入 main，仍须随 WP1～WP4 接受缪海南的正式 WP5 独立复验。
...
- schema / 依赖：沿用 schema v34，无新迁移、第三方依赖、HTTP API、Agent 路由、LangGraph 语义或平台写动作；WP3 已先由本地 main `d35bfe1` 预留下一空闲 v35，再在 `6f0b116` 增加运行迁移候选。
...
- schema：v34 additive 新增 `readonly_import_manifests`、`readonly_import_row_issues`、`readonly_field_evidence` 三张表及证据不可变触发器；支持 v33→v34 前向迁移，不重建既有表。v34 已合入 main；v35 已由本地 main 预留给 WP3，下一空闲 schema 为 v36。
