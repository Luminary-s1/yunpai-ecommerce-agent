...
- M9-R PR #19 本地候选以 schema v39 修复 item 写入隔离：库存表安全重建为 item 专属/未知
  身份两组部分唯一索引；订单保持 `(tenant,connector,store,external_order_id)` 单一聚合根，
  `commerce_order_lines.item_id` 从旧订单头回填并承担行级归属。v36→v39 迁移保留订单头、
  ...
  PR #26 合入 `main@1ee68cb` 的唯一权威 `CONTRIBUTING.md` 表；M9-R v39 运行实现仍未
  提交到 PR #19，因此不得视为已发布 schema 或正式独立验收通过；见 E-20260824-001。
...
- 状态：WP1 因是 M7-R 与 M8-R～M10-R 共用基建而先行合入；PR #20 只承载 WP2～WP4，
  固定验收 head `ece61e1` 已合入 `main` 为 `f6bb47c`。F-323 的代码级、本机技术里程碑
  已通过 G-M7R-WP5-001 / G-M7R-ALL-001。
- 验收：缪海南原独立报告保持原文和签署边界；实施方补充报告补齐 base 中 WP1 与 head 中
  WP2～WP4 的组合矩阵、mutation 红→绿和浏览器证据但不代签。用户转交的第二轮独立
  double-check 复核报告/截图哈希、提交拓扑、命令链与截图内容。干净 detached merge tip
  聚焦 `104 passed in 16.74s`、全量 `1035 passed, 24 warnings in 421.29s`，无
  failed/skipped/xfailed；静态和 project-to-act 门禁通过。见 E-20260820-001。
- 版本与边界：运行时包版本仍为 `0.30.0`，schema 当前为 v35；本次没有发布版升级。该 Gate
  不覆盖真实淘宝/天猫字段全量确认、真实经营结论、生产发布/权限/写能力或 M8-R～M10-R。

## M7-R WP4 数据准备度、只读工作台与 Demo（未单独升应用版）

- 状态：开发候选 `fe828a0` 已纳入 PR #20 验收 head `ece61e1`，通过 WP5 后合入 `main`
  为 `f6bb47c`；代码级本机结论见 E-20260820-001。
...
- 验证：初始候选 E-20260819-001；独立反馈与空白店铺查询收口 E-20260819-002。用户转交的
  独立报告记录 47/47 门禁外探针通过；开发方以先红后绿回归统一六个 GET 的 Query 约束。
  候选聚焦 `10 passed`、WP1～WP4 关联 `109 passed`、全量 `1035 passed, 24 warnings`
  （335.51 秒），无 failed/skipped/xfailed；24 条均为既有 Traffic Lab 重复 Operation ID
  warning。最终 WP5 与 merge-tip 复验见 E-20260820-001；E-001 浏览器页面证据已归档。
  真实平台样本、真实经营结论与生产放行未获批准。

## M7-R WP3 商品身份与对账（未单独升应用版）

- 状态：开发候选 `6f0b116` 已纳入 PR #20 验收 head `ece61e1`，schema v35 运行迁移与代码均已随 `f6bb47c` 合入 `main`，并通过 E-20260820-001 的 WP5。
- schema / 依赖：v35 additive 新增 `readonly_canonical_products`、`readonly_product_mapping_events`、`readonly_product_reconciliation_runs` 和 `readonly_product_reconciliation_rows`，四表均有 UPDATE/DELETE 不可变触发器及 tenant/store 复合外键；v34 可前向迁移且既有表不重建。无新第三方依赖、HTTP API、Agent 路由、LangGraph 语义或平台写动作。
- 兼容性：canonical 商品以 tenant/store/internal part 唯一；平台 SKU 映射确认、显式改判和撤销只追加，带完整载荷幂等键和 `expected_version` 乐观锁。当前只接受 `product-identity-v1`，未知策略版本读侧 fail closed。标题、商家编码或唯一候选都不自动正式绑定；只有有效人工确认产生 matched 与稳定料号。
- 来源与下游：`reconcile_domain` 只经 catalog/inventory/order 公开服务读取，WP2 readonly source ID 通过唯一 `source_manifest_key` 回到 WP1 manifest，默认 operational 排除 Demo，无法解析的 readonly 来源在 scoped 视图 fail closed。M9-R/M10-R 只可消费 matched 行固化的 canonical ID、internal part、run/policy/snapshot 证据。
- 灾备：v35 继续要求 manifest 精确匹配 schema；升级前以 v34 程序完成停机备份，升级后恢复业务写入前生成并验证 v35 全量备份，旧程序与旧归档保留到隔离恢复演练通过。
- 验证：E-20260818-002；初始缺实现收集退出 2，预期版本/证据键审计和 Demo 来源隔离均有先红后绿证据。聚焦 `17 passed`、WP1～WP3 `93 passed`、关联回归 `184 passed`，候选全量 `1025 passed, 24 warnings`（391.62 秒），无 failed/skipped/xfailed；24 条为既有 FastAPI 重复 Operation ID warning。首次全量被桌面会话畸形 `NO_PROXY` 的 `httpx.InvalidURL` 污染，隔离全部 proxy/bypass 环境变量后单例与全量均通过，未修改产品代码掩盖环境错误。最终 WP5 与 merge-tip 复验见 E-20260820-001。

## M7-R WP2 报表适配与规范化写入（未单独升应用版）

- 状态：`generic-cn-v1` 开发候选 `a5d02ed` 已纳入 PR #20 验收 head `ece61e1`，通过 WP5 后随 `f6bb47c` 合入 `main`。
- schema / 依赖：沿用 schema v34，无新迁移、第三方依赖、HTTP API、Agent 路由、LangGraph 语义或平台写动作；下游 WP3 的 v35 运行迁移已在 `main`。
- 兼容性：additive 导出 `REPORT_ADAPTERS`、`ReportImportRequest`、`ReportImportJob` 与 `ReadonlyReportIngestionService`。`generic-cn-v1` 接受严格 UTF-8 CSV 和受控 XLSX，旧 WP1 契约保持可用；同版本不同内容与旧来源在领域写入前拒绝。新的真实平台字段契约必须使用独立 mapping version，不得静默改写已冻结输入语义。
- 样本边界：仓库及会话附件没有经授权的淘宝/天猫真实受控导出；当前中文别名是通用候选，不代表真实平台列已完整确认。原始文件上传仍由受控存储边界负责，导入服务只接收内容和已批准引用。
- 验证：E-20260818-001；初始缺实现收集退出 2，订单子事实覆盖、Excel 日期序列和 Demo 显式回执三项回归均先红后绿。WP1+WP2 聚焦 `77 passed`，候选全量 `1008 passed, 24 warnings`，无 failed/skipped/xfailed；24 条为既有 FastAPI 重复 Operation ID warning。最终 WP5 与 merge-tip 复验见 E-20260820-001。

## M7-R WP1 只读经营数据统一契约（未单独升应用版）

- 状态：WP1 因是 M7-R 与 M8-R～M10-R 共用基建而先行合入 `main`；其固定提交已作为 PR #20 base 的祖先纳入 WP5 组合验收并通过 E-20260820-001。当前运行时包版本保持 `0.30.0`，不得据此声明生产放行。
- schema：v34 additive 新增 `readonly_import_manifests`、`readonly_import_row_issues`、`readonly_field_evidence` 三张表及证据不可变触发器；支持 v33→v34 前向迁移，不重建既有表。v34 已合入 main；后续占号以 `CONTRIBUTING.md` 第 9 节权威登记为准。
...
- 验证：初始 E-20260817-003，独立反馈收口 E-20260817-004，既有 M4/知识库测试欠账收口 E-20260817-005，main 集成见 E-20260817-007。WP1 最终聚焦 `19 passed`，迁移/Traffic Lab/灾备/CLI 组合 `61 passed`；三项 red-first 契约覆盖 allowlist 值携带 PII、证件/邮箱/邮编字段名和调用方伪造质量计数。随后原七项欠账定点 `7 passed`、skip/xfail 相关集合 `59 passed`、关联 `108 passed`；隔离 main 集成树全量 `950 passed, 24 warnings`，无 failed/skipped/xfailed。E-005～007 未改变依赖、租户权限、D-034 语义权威或 WP1 冻结范围；最终组合验收与 merge-tip 复验见 E-20260820-001。
