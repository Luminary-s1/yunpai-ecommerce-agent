...
- 兼容性说明（0.30.0 增量）：无 schema/API 契约变化；后台页面新增适配器与灰度面板、发布表单可选夜间/SOP 白名单字段（不填时请求与 0.29.0 相同）
- 最后更新：2026-07-27
...
- 兼容性说明（0.29.0 增量）：schema v25 为 release_policies 增加 4 个可空列（additive 前向迁移）；`assignment()` 返回的 policy 增加 `configured_mode`/`night_watch_active` 且 `mode` 为生效模式（未配置夜间窗时与原值一致）；策略创建请求新增可选 night/sop_allowlist 字段，旧请求不受影响
- 最后更新：2026-07-27

## 又前一版本

- 版本号：`0.28.0`
- 发布状态：商品/SKU 顾问本机候选；生产放行阻塞
- 兼容性说明（0.28.0 增量）：无 schema 变化；context bundle 新增 `product_advisor` 段与 `catalog_item` 证据类型（无店铺或无命中时为空段，既有消费方不受影响）
- 最后更新：2026-07-27
...
| 版本 | 状态 | 主要变更 | 验证证据 |
|---|---|---|---|
| `0.30.0` | 当前本机候选 | 工作台：适配器能力面板、知识/SOP 灰度状态面板、夜间值守与 SOP 白名单策略创建/展示 | E-20260727-006：后台 5 项测试、页面单脚本 JS 解析、浏览器渲染检查 |
