...
- 版本号：`0.22.6`
- 发布状态：管理后台响应式视觉优化本机候选；生产放行阻塞
- 兼容性说明：Python 3.11+、单机 SQLite schema v22，历史 schema 可前向迁移；sessions 新增 `source_type/source_reference` 并回填 virtual/evaluation/taobao 来源。后台 API 默认 `operational` 范围排除 simulation/evaluation，并支持 `simulation/evaluation/all` 显式切换；顾客直测仍须显式启用并仅限回环，正式 `/v1/chat` 客户鉴权保持兼容；0.22.1 场景证据契约保持兼容，模拟运行仍必须显式确认 virtual，固定 `production_claim=false`
- 最后更新：2026-07-23
...
| 版本 | 状态 | 主要变更 | 验证证据 |
|---|---|---|---|
| `0.22.6` | 当前本机候选 | 约束后台内容区、概览内容流、表格、会话、消息和测试结果的尺寸与内部滚动；390px 导航保持滑动且隐藏原生滚动条 | E-20260723-004：后台/API 定向 7 项测试、桌面/390px 浏览器与 console 0 error/warning |
