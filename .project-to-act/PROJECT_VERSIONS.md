...
| 版本 | 状态 | 主要变更 | 验证证据 |
|---|---|---|---|
| `0.26.0` | 当前本机候选 | schema v24 通用 `staged_rollouts`；知识灰度 begin/调量/complete/rollback 生命周期；检索按会话稳定分桶仲裁 baseline/candidate；无分桶单元路径固定基线；管理 API | E-20260727-002：18 项灰度/迁移测试、43 项治理/检索/Agent/灾备回归、chat 会话分桶一致性 |
