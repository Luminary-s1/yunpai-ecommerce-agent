...
- 占号状态：PR #10 已合入 main `1906365`，schema **v33** 在 `main`（knowledge_key 唯一索引 + retrieval_logs）。F-322 **v32** 已在 main。PR #11 的 `_apply_v31`（workspace 会话表）仍占用中、未合入。合 #11 时必须 31+32+33 三块并存，扫描 `MERGE-GATE PR-11`。M7-R WP1 已以提交 `ec19061` 权威占用 **v34**；下一空闲号 **35**。
- 最后更新：2026-08-17

## M7-R WP1 只读经营数据统一契约（未单独升应用版）

- 状态：WP1 已形成开发自测候选，仍等待缪海南在干净状态执行 WP5 独立复验；当前运行时包版本保持 `0.30.0`，不得据此声明 M7-R 或生产放行通过。
- schema：v34 additive 新增 `readonly_import_manifests`、`readonly_import_row_issues`、`readonly_field_evidence` 三张表及证据不可变触发器；支持 v33→v34 前向迁移，不重建既有表。v34 已占用，下一空闲 schema 为 v35。
