...
- 版本号：`0.23.0`
- 发布状态：营销与利润模块本机候选；生产放行阻塞
- 兼容性说明：Python 3.11+、单机 SQLite schema v23，历史 schema 可前向迁移；新增营销日指标、内容草稿、来源费用、结算单和对账任务。营销诊断与利润工具均为只读，内容只保存草稿且禁止发布；预算、竞价、总账、税务、结算和资金指令不在能力范围。后台 API 默认 `operational` 范围排除 simulation/evaluation，并支持 `simulation/evaluation/all` 显式切换；顾客直测仍须显式启用并仅限回环，正式 `/v1/chat` 客户鉴权保持兼容；场景证据契约保持兼容，模拟运行仍必须显式确认 virtual，固定 `production_claim=false`
- 最后更新：2026-07-23
...
| 版本 | 状态 | 主要变更 | 验证证据 |
|---|---|---|---|
| `0.23.0` | 当前本机候选 | schema v23 版本化营销日指标、内容草稿有限事实检查、费用与结算单；提供投放诊断、管理利润估算、差异任务人工流转、两个 Agent 只读工具、控制台工作台和 D14/D15 真实输入输出 | E-20260723-005：15/15 虚拟场景、营销/财务 API 回归、后台/API 定向 10 项测试、页面 JS 解析 |
