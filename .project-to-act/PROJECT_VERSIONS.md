...
| 版本 | 状态 | 主要变更 | 验证证据 |
|---|---|---|---|
| `0.25.0` | 当前本机候选 | 信封归一化 `message_kind` 与多消息类型：非文本入站记录 + 脱敏占位符 + 运行时强制转人工；context checkpoint 前白名单对抗敌意载荷验证；跨店铺/跨租户不可合并契约与落库器测试 | E-20260727-001：2 项契约用例 × 2 适配器、非文本转人工双适配器运行时、context snapshot 白名单、落库器租户隔离、61 项渠道回归、channel_sdk 分支覆盖 90–100%、全量 261 通过 |
