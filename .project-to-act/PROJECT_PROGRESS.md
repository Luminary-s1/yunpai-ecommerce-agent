...
- M7-R WP1 的统一只读数据契约开发候选见 F-323 / E-20260817-003/004，既有 M4/知识库失败及 skip/xfail 测试欠账收口和全绿开发基线见 E-20260817-005；独立技术复验反馈的值级 PII、敏感字段名和质量计数入口已收口，WP2 与 M8-R～M10-R 可据此开始契约适配骨架。真实数据域和平台字段仍须由 WP2 基于受控样本实现，`parsed_rows` 必须来自真实解析观察；这些证据不代表后续工作包、正式 WP5、真实平台接入或生产放行。
...
工作包的负责人和交叉独立验收负责人（WP5）以各 WORKBENCH 的稳定分工表及 D-045 为准。
