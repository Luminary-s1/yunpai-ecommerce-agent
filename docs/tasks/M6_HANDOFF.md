...
| 承接人 | 工作包 | 方向 | 工时 | 进度 |
|---|---|---|---:|---:|
| 缪海南 | 竞品数据模型定义与结构化管理服务 | 数据层 | 16h | 15% |
| 胡磊 | 竞品数据采集与结构化管理（已重新界定范围） | 取数适配 | **6h** | 0% |
| 胡磊 | 竞品对比分析引擎开发 | 分析层 | 20h | 25% |
| 胡磊 | 竞品多维对比分析与报告生成引擎 | 分析层 | 20h | 25% |
| 闫睿涵（模块负责人） | 竞品数据准确性与报告质量测试 | 验收测试 | 8h | 0% |
...
> 工时从 14h 压到 6h，承接人改为胡磊。模块合计 78h → 70h。
...
两位现在都是三模块并行，合计都在 60 小时以上。**M6 的开发包优先级最高**，
因为它是唯一你们写业务代码的模块，另外两个模块里你们是验收角色，
而验收本来就要等对方交付。

**缪海南**（M6 16h + M5 数据层 34h + M4 端到端交付 18h）
...
4. M4「客服对话引擎端到端联调与交付」排最后：它是 M4 的独立验收出口，
   要等模块负责人的意图路由做完才能写 D18 场景。完整说明见
   `docs/tasks/M4_HANDOFF.md`

M5 两个包的顺序与依赖见 `docs/tasks/M5_WORKBENCH.md` 各包的
「子任务交付契约」，`dependencies` 字段里写死了。

**胡磊**（M6 46h + M4 评测调优 16h + M5 验收 8h）

1. M6 两个分析包优先——它们已经 25%，且不依赖数据层的新接口，现在就能全速推
2. M4「客服模块效果评测与调优」可以插空做，它不依赖任何人；但**指标口径与判定
   标准要和模块负责人一起定**，别单方面拍板
3. 等缪海南的数据模型包定稿后，做「竞品数据采集与结构化管理」（6h 取数适配层）
4. M5 验收测试最后做，要等缪海南、模块负责人和谢良璇都交付

M4 的端到端交付已改派给缪海南，你不承接；但你接了 M4 的评测调优，
说明见 `docs/tasks/M4_HANDOFF.md` 第 2 节。
...
- **分析侧取数适配（6h）**——只返回已批准 match 绑定的数据，未批准数据在入口
