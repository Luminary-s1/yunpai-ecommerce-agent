...
**缪海南**（M6 30h + M5 数据层 34h）

1. 先做 M6「竞品数据采集与结构化管理」，把 Schema 定下来解开胡磊的阻塞
2. 再做 M5「运营数据解析与上传服务开发」——它定的字段级错误结构是
   M5 另一个包的前提，也在关键路径上
3. M5「运营数据接口与解析模块开发」排最后，它要等上一条定稿
4. 回头收尾 M6「竞品数据模型定义与结构化管理服务」

M5 两个包的顺序与依赖见 `docs/tasks/M5_WORKBENCH.md` 各包的
「子任务交付契约」，`dependencies` 字段里写死了。

**胡磊**（M6 40h + M5 验收 8h + M4 端到端交付 18h）

1. M6 两个分析包优先，等缪海南 Schema 定稿后全速做
2. M4「客服对话引擎端到端联调与交付」的前两项（流式实跑证据、D17 场景）
   不依赖任何人，可以插空做；第三项 D18 要等 M4 的置信度路由落地
3. M5 验收测试最后做，要等缪海南和模块负责人交付

M4 那个包的完整说明见 `docs/tasks/M4_HANDOFF.md`。
