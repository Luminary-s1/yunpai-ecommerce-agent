...
面向承接**工作包 5「运营分析与报告生成开发」**的同学。读完这一份加根目录的
...
| 待分配 | 运营内容与数据服务验收测试 | 验收测试 |

按模块任务书要求，**开发与测试必须由不同成员承担**。工作包 1 覆盖整个模块，
所以它的承接人既不能是工作包 5 的开发者，也不能是模块负责人本人——需要第三个人。
在它分配下来之前，每个 PR 仍需自带定向测试（见第 6 节）。
...
如果这个包由两位同学一起做，按下面切，中间只有一次同步：

| | 承接内容 | 工时 |
|---|---|---:|
| A（计算线） | 环比计算、同比计算、优化建议绑定具体数值 | 12 |
| B（配置与输出线） | 阈值配置文件、异常预警独立章节、报告导出 | 12 |

- **第 1 周**：A 做环比/同比，**把新增的输出结构定下来后在群里同步一次**；
  B 同期做阈值配置文件（自洽，不依赖 A）
- **第 2 周**：A 做建议绑定数值；B 做异常预警章节（要同时消费 A 的环比结果
  和自己的阈值配置，所以排在同步之后）
- **第 3 周**：B 做报告导出，放最后——导出要把前面新增的章节都包进去

一个人做的话，顺序是：环比 → 同比 → 阈值配置 → 预警章节 → 建议绑定数值 → 导出。
...
全量约 9 分钟，当前用例数 389（`pytest --collect-only -q` 可确认）。
