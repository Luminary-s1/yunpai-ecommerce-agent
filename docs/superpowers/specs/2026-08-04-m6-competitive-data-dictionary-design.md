...
> 文档版本：`M6-DD-1.0`
> 日期：2026-08-04
> 状态：字段设计完成，代码实现与验收尚未开始
> 适用范围：M6 工作包 2「竞品数据采集与结构化管理」和工作包 3「竞品数据模型定义与结构化管理服务」
...
- D-025：竞品事实必须先经过可解释同款评分和人工裁决。只有 approved 匹配绑定的
  价格、卖点和脱敏聚合口碑才能进入分析结论与 Agent 建议。
- 不保存评论者身份或原始评论内容。
...
管理视图可以返回 pending/rejected/approved；分析与 Agent 视图必须强制
`match.status=approved`，不能由客户端关闭该门禁。
...
当前代码 `SCHEMA_VERSION = 25`。实现前必须在团队群中占用新的 schema 版本号，
文档不预占具体数字。
...
迁移必须使用 `_ensure_column`，并检查数据库类中不存在重复的 `_apply_vN` 方法名。
...
1. 团队确认本字段字典，尤其是自定义维度和动态评分语义。
2. 在群内占用新的 schema 版本号。
